...
4. Change tag to the _fabric-baseimage_: `docker tag rikmoedt/fabric-baseimage:latest hyperledger/fabric-baseimage:latest` (note: be sure you do not have already an image with the same tag, in that case, rename first the current _hyperledger/fabric-baseimage_ to something else)
